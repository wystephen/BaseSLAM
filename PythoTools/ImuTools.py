...
#
#     q[0] = ql[0] * qr[0] - ql[1] * qr[1] - ql[2] * qr[2] - ql[3] * qr[3]
#     q[1] = ql[0] * qr[1] + ql[1] * qr[0] + ql[3] * qr[2] - ql[2] * qr[3]
#     q[2] = ql[0] * qr[2] + ql[2] * qr[0] + ql[1] * qr[3] - ql[3] * qr[1]
#     q[3] = ql[0] * qr[3] + ql[3] * qr[0] + ql[2] * qr[1] - ql[1] * qr[2]
#
#     q_norm = q[0] * q[0] + q[1] * q[1] + q[2] * q[2] + q[3] * q[3]
#     q_norm = math.sqrt(q_norm)
#
#     if abs(q_norm - 1.0) > 0.01:
#         print('warning in quaternion product')
#         for i in range(4):
#             q[i] = q[i] / q_norm
#
#     return q
...
# def test_QuaternionProductTEST():
#     q1 = dcm2q(euler2R(np.asarray([0.5, 0.2, 2.0])))
#     q2 = dcm2q(euler2R(np.asarray([0.9, 2.0, 1.0])))
#
#     oq = quaternion_product(q1, q2)
#     oqt = dcm2q((q2dcm(q1)).dot(q2dcm(q2)))
#
#     print('oq', oq, 'oqt', oqt)
#     print('diff:', oq - oqt)
#     print('real diff', np.linalg.inv(q2dcm(oq)).dot(q2dcm(oqt)))


def hat(u):
    assert u.shape[0] is 3
    # U = np.zeros([3,3])
    U = np.asarray([
        [0.0, -u[2], u[1]],
        [u[2], 0.0, -u[0]],
